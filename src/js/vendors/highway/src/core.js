...
export default class Core extends Emitter {
  ...
  constructor({ renderers, transitions } = {}) {
    ...
    // リロード時にcurrentCountを復元できるように保存
    window.addEventListener('pagehide', () => {
      sessionStorage.setItem('historyCount', this.historyCount);
    });
  }

  attachAll(context = document) {
    const links = context.querySelectorAll(this.getSelectorLink())
    const detach = this.attach(links);
   ...
   */
  attach(links) {
    const destroyLinks = [...links].map((link) => {
      link.addEventListener('click', this._navigate);
      const destroy = this.listenPrefetch(link);
  ...
  popState(e) {
    ...
    // 履歴戻る/進む判別
    if(e.state?.count > this.historyCount) {
      this.direction = 'forward';
    } else {
      this.direction = 'back';
    }
    this.historyCount = e.state?.count || 0;
  ...
  pushState() {
    if (!this.popping) {
      window.history.pushState({ count: ++this.historyCount }, '', this.location.href);
    }
  }

  async fetch(url, isTransition = false) {
    if (this.cacheFetch.has(url)) {
      return this.cacheFetch.get(url);
    } else {
      ...
      if (response.status >= 200 && response.status < 300) {
        this.cacheFetch.set(url, response);
        return response;
      } else if (isTransition) {
        window.location.href = url;
      } else {
        this.cacheFetch.set(url, null);
        return null;
      }
    }
  }
   ...
   */
  async fetchPage(url, isTransition = false) {
    if (this.cache.has(url)) {
      return this.cache.get(url);
    } else {
      const promiseProperties = (async () => {
        // We wait till all our Promises are resolved.
        const result = await this.fetch(url, isTransition);
        ...
        const properties = this.Helpers.getProperties((await result.text()));

        // We cache our result
        // eslint-disable-next-line
        this.cache.set(url, properties);

        return properties;
      })();

      this.cache.set(url, promiseProperties);
  ...
  listenPrefetch(link) {
    if (this.cache.has(link.pathname)) {
      return () => {};
    } else if ('routerDisablePrefetch' in link.dataset) {
      const listener = () => {
        this.fetchPage(link.pathname);
      };
      ...
      return () => {
        link.removeEventListener('mouseenter', listener, {
          once: true,
        });
      };
    } else {
  ...
  getIntersectionObserver(threshold = 0) {
    let observer;
    if (this.cacheIntersectionObserver.has(threshold)) {
      observer = this.cacheIntersectionObserver.get(threshold);
    } else {
      observer = new IntersectionObserver(entries => {
        entries.forEach(entry => {
          if (entry.isIntersecting) {
            const { target } = entry;
            this.fetchPage(target.pathname);
      ...
      this.cacheIntersectionObserver.set(threshold, observer);
    }
    return observer;
  }
  ...
  async beforeFetch() {
    ...
    const results = await Promise.all([
      this.fetchPage(this.location.pathname, true),
  ...
  getSelectorLink(pathname = window.location.pathname) {
    return `a[href]:not([href^="#"]):not([href^="${pathname}#"]):not([target]):not([download]):not([data-router-disabled])`;
  }
}
